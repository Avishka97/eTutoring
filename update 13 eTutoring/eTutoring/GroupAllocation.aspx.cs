...
namespace eTutoring
{
    public partial class GroupAllocation : System.Web.UI.Page
    {
            ...
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                try
                {
                    using (SqlConnection sqlCon = new SqlConnection(ConnectionString))
                    {
                        ...
                        sqlCmd.Parameters.AddWithValue("@T_ID", (row.FindControl("DropDownList1") as DropDownList).Text.Trim());
                        sqlCmd.Parameters.AddWithValue("@S_ID", (row.FindControl("lblSNo") as Label).Text.Trim());
                        sqlCmd.ExecuteNonQuery();
                        GridView1.EditIndex = -1;
                        PopulateGridview();
                        sqlCon.Close();
                        lblSuccessMessage.Text = "Tutors Assigned";
                        lblErrorMessage.Text = "";


                        string query1 = "select Email from Student where S_ID=@S_ID";
                        SqlCommand sqlCmd2 = new SqlCommand(query1, sqlCon);
                        sqlCmd2.Parameters.AddWithValue("@S_ID", (row.FindControl("lblSNo") as Label).Text.Trim());
